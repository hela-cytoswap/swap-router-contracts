...
contract OracleSlippageTest is OracleSlippage {
    mapping(address => mapping(address => mapping(uint24 => ICytoswapV3Pool))) private pools;
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view override returns (ICytoswapV3Pool pool) {
        pool = pools[tokenA][tokenB][fee];
    }

    function testGetBlockStartingAndCurrentTick(ICytoswapV3Pool pool)
        external
        view
